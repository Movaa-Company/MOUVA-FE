  ...
  useEffect(() => {
    const luhn = (num: string) => {
      let arr = (num + '')
        ...
        .map((x) => parseInt(x));
      let sum = arr.reduce(
        (acc, val, i) => acc + (i % 2 ? ((val *= 2) > 9 ? val - 9 : val) : val),
        0
      );
      return sum % 10 === 0;
    };
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden flex flex-col md:flex-row bg-[#F6FBF7]">
      ...
      <div className="w-full md:w-1/4 p-4 md:p-6 flex flex-row md:flex-col gap-2 md:gap-8 bg-[#F6FBF7]">
        <Button
          variant={tab === 'transfer' ? 'default' : 'outline'}
          className="justify-start"
          onClick={() => setTab('transfer')}
        >
          {' '}
          <span className="mr-2">🛩️</span> Transfer{' '}
        </Button>
        <Button
        ...
        <Button
          variant={tab === 'palpay' ? 'default' : 'outline'}
          className="justify-start"
          onClick={handlePalPay}
          disabled
        >
          {' '}
          <span className="mr-2">🅿️</span> PalPay{' '}
        </Button>
      </div>
        ...
        <div className="w-full max-w-lg box-border">
          ...
          {tab === 'transfer' && (
            <>
              ...
              <Button
                className="w-full bg-movaa-primary hover:bg-movaa-dark text-white"
